...
image_file = Path(image_file) if image_file else None
if not image_file or not image_file.exists():
	print(f"❌ Error: The image file was not found or not specified.")
	sys.exit(1)

# Load the Excel file
data = pd.read_excel(excel_file, sheet_name=sheet_name)

# Load the message if it exists, otherwise skip
if message_file.exists():
    with open(message_file, 'r', encoding='utf-8') as file:
        message = file.read().strip()
    has_message = bool(message)
else:
    print("⚠️ No message file found — only image will be sent.")
    message = ""
    has_message = False
...
for index, row in data.iterrows():
    ...
    try:
        ...
        # Copy image to clipboard (Windows PowerShell)
        subprocess.run([
            "powershell", "-command",
            f"Set-Clipboard -Path '{image_file}'"
        ], shell=True)
